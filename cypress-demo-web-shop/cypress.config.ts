...
export default defineConfig({
  ...
  //Whether Cypress will trash assets within the downloadsFolder,
  //screenshotsFolder, and videosFolder before tests run with cypress run.
  ...
  viewportHeight: 660, //Height in pixels for the application under tests' viewport. 
  viewportWidth: 1000, //Width in pixels for the application under tests' viewport.
  ...
  //Can be configured to apply to cypress run or cypress open separately.
  //If you want to configure retry attempts on a specific test or suite,
  //you can set this by using the test's/suite's configuration.
  retries: {
  ...
  e2e: {
    ...
    setupNodeEvents(on, config) {
      ...
      //It is very important to return the updated config object to the caller,
      //so Cypress knows to use the changes configuration.
      return config;
    },
  },
})
